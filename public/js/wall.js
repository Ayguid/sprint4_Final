...
function newPost(){
    ...
    $.ajax({
        ...
        error: function(){
            $('#errorMessageModal').modal('show');
            $('#errorMessageModal #errors').html('Whoops ...algo salio mal!');
            $(form_name + ' .loading-post').hide();
        }
    });
    ...
    if (!fetch_end) {
        ...
        $.ajax({
            ...
            error: function () {
                /*
                $('#errorMessageModal').modal('show');
                $('#errorMessageModal #errors').html('Whoops ...algo salio mal when loading your wall!');*/
                $('.post-list-top-loading').hide();
                $('.post-list-bottom-loading').hide();
                fetch_end = false;
            }
...
function deletePost(id){
    ...
    BootstrapDialog.show({
        title: 'Borrado Post!',
        message: 'Estas seguro que queres borrar el post ?',
        buttons: [{
            label: "Si!",
            cssClass: 'btn-danger',
            action: function(dialog) {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        dialog.close();
                        if (response.code == 200){
                            $('#panel-post-'+id).html("Post borrado!");
                        }else{
                            $('#errorMessageModal').modal('show');
                            $('#errorMessageModal #errors').html('Whoops ...algo salio mal!');
                        }
...
function submitComment(id){
    ...
    if (comment.trim() == ''){
        $('#errorMessageModal').modal('show');
        $('#errorMessageModal #errors').html('Escribite un comentario!');
    }else {
        $.ajax({
...
function removeComment(id, post_id){
    ...
    BootstrapDialog.show({
        ...
        message: 'Estas seguro que lo queres borrar ?',
        buttons: [{
            label: "SI!",
            cssClass: 'btn-danger',
            action: function(dialog) {
                ...
                $.ajax({
                    ...
                    success: function(response){
                        dialog.close();
                        if (response.code == 200){
                            $('#post-comment-'+id+' .panel-body').html("<p><small>Comentario borrado!</small></p>");
                            $('#panel-post-'+post_id+' .comments-title').html(response.comments_title);
                        }else{
                            $('#errorMessageModal').modal('show');
                            $('#errorMessageModal #errors').html('Whoooopss...Algo Salio Mal!!');
                        }
                    },
                    error: function(){
                        dialog.close();
